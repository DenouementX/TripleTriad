var deck = [
	{"id": 0, "name": "Empty", "ranks": [0,0,0,0], "element": "NEUTRAL", "level": 1},
...
export function getCardStats(cardID) {
	return deck[cardID].ranks;
}

export function getCardStatsUp(cardID) {
	return getCardStats(cardID)[0];
}

export function getCardStatsRight(cardID) {
	return getCardStats(cardID)[2];
}

export function getCardStatsDown(cardID) {
	return getCardStats(cardID)[3];
}

export function getCardStatsLeft(cardID) {
	return getCardStats(cardID)[1];
}

// Given a unique card ID, return the  information
export function getCardImageURL(cardID, RELATIVEPATH) {
    // Get into 000, 001, 002 format
    let formattedNumber = cardID.toLocaleString('en-US', {
        minimumIntegerDigits: 3,
        useGrouping: false
    });
    return RELATIVEPATH + formattedNumber + '.png';
}

export function generateRandomCard(min, max) { // Min and max included 
...
export function generateRandomHand() {
	var hand = [];
	while(hand.length < 5) {
		var card = generateRandomCard(1,110);
		if (!hand.includes(card)) { // Ensures no duplicate cards in the same hand
			hand.push(card);
		}
	}
	return hand;
